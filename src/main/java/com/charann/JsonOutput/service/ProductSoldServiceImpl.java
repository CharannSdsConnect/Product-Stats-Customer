...
public class ProductSoldServiceImpl implements ProductSoldService {
    ...
    public ProductSold addSoldProduct(ProductSold productSold) {
        ...
        if(remainingStocks.getQuantity() - productSold.getQuantity() >= 0) {
            ...
            productSold1.setCustomer(productSold.getCustomer());
            productSold1.setCustId(productSold.getCustId());
...
//        }));
    ...
    public String getCustomerReport() {
        ...
        productSoldList.forEach(product-> {
            if(customerReportRepo.existsByCustId(product.getCustId())) {
                CustomerReport customerReport = customerReportRepo.getByCustId(product.getCustId());
                ...
                customerReport.setCustomer(product.getCustomer());
                customerReport.setCustId(product.getCustId());
    ...
    public void generateCustomerReport(HttpServletResponse response) throws IOException {
        List<CustomerReport> customerReportList = customerReportRepo.findAllByOrderByPriceDesc();
        ...
        for(CustomerReport customer:customerReportList) {
            ...
            dataRow.createCell(3).setCellValue(customer.getPoints());
